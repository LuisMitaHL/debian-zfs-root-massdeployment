...
stage_luks() {
  ...
  for d in "${DISKS[@]}"; do
    ...
    if (( APPLY )); then
      ...
      CRYPTTAB_LINES+=("$name UUID=$uuid none luks,discard,initramfs")
    else
      printf '  would run: cryptsetup luksFormat --type luks2 ... %s\n' "$part" >&2
      printf '  would run: cryptsetup open %s %s\n' "$part" "$name" >&2
      CRYPTTAB_LINES+=("$name UUID=<luks-uuid> none luks,discard,initramfs")
    fi
...
stage_verify() {
  ...
  if [[ "$CRYPT" == "yes" ]]; then
    ...
    for (( i = 0; i < ${#DISKS[@]}; i++ )); do
      if grep -qE "^zfs${i}[[:space:]]+UUID=[0-9a-f-]{36}[[:space:]]" "$MNT/etc/crypttab"; then
        ok "crypttab entry zfs$i is keyed by LUKS UUID"
      else
        warn "crypttab entry zfs$i is not keyed by UUID — a device rename would break the boot"
        bad=1
      fi
    done
  fi
