...
# scripts, and the per-host profiles. It also runs sshd: at boot an 8-char root password
# is autogenerated and the live user's console login shows every interface address plus
# that password, so the operator can work over `ssh root@<addr>`. It does NOT carry the golden stream — that lives on a
...
zstd
rsync
# Remote access to the live environment: sshd plus the tools the login banner needs.
# (The root password itself is generated per boot by live-ssh-setup.service.)
openssh-server
iproute2
EOF
...
# SSH into the live environment: sshd accepts the per-boot root password, which the
# console banner shows at the live user's login. See live/ for the sources.
install -m 0755 "$REPO/live/live-ssh-setup.sh"     "$LBWORK/config/includes.chroot/usr/local/sbin/live-ssh-setup.sh"
install -m 0644 "$REPO/live/live-ssh-setup.service" "$LBWORK/config/includes.chroot/etc/systemd/system/live-ssh-setup.service"
...
# sshd plus the per-boot root-password generator must be enabled, or the console
# banner would advertise SSH access that is not listening.
cat > "$LBWORK/config/hooks/normal/0110-live-ssh.hook.chroot" <<'EOF'
#!/bin/sh
set -eu
systemctl enable ssh.service live-ssh-setup.service
test -f /etc/ssh/sshd_config.d/99-live-root.conf
test -x /usr/local/sbin/live-ssh-setup.sh
EOF
chmod +x "$LBWORK/config/hooks/normal/0110-live-ssh.hook.chroot"
